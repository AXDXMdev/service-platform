...
export default function ProviderVerificationPage() {
  ...
  const submit = async () => {
    ...
    if (!companyName.trim() || !city.trim() || proofs.length === 0) {
      setMessage("Bitte Name, Stadt und mindestens einen gueltigen Nachweis-Link ausfuellen.")
      return
    }
    if (contactEmail && !isValidEmail(contactEmail)) {
      setMessage("Bitte eine gueltige Kontakt-E-Mail eingeben.")
      return
    }
    if (!privacyAccepted || !verificationDisclaimerAccepted) {
      setMessage("Bitte Datenschutz- und Verifizierungshinweise bestaetigen.")
      return
    }
    ...
    setMessage("Anfrage eingereicht. Das Admin-Team prueft deine Nachweise.")
  }
      ...
      <div className="mx-auto max-w-4xl animate-float-up">
        <section className="card-surface rounded-2xl p-8 sm:p-10">
          ...
          <div className="panel-muted mt-5 rounded-xl p-4 text-sm leading-6 text-slate-700 dark:text-slate-200">
            ...
            <p className="mt-2">
              Hilfinio prueft eingereichte Nachweise und Profilangaben nur im Rahmen eines
              Plausibilitaets- und Dokumentenchecks. Der Verifizierungsstatus ist keine
              Garantie fuer Qualitaet, Bonitaet, fortbestehende Berechtigung oder die
              rechtliche Zulaessigkeit aller angebotenen Leistungen.
            </p>
          </div>
          ...
          <div className="mt-5 space-y-3">
            <label className="flex items-start gap-3 text-sm text-slate-700 dark:text-slate-200">
              ...
              <span>
                ...
                gelesen und bin mit der Verarbeitung meiner Angaben und Nachweise fuer das
            ...
            <label className="flex items-start gap-3 text-sm text-slate-700 dark:text-slate-200">
              ...
              />
              <span>
                Ich bestaetige, dass meine Angaben richtig sind und dass ein
