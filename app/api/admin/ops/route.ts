...
export async function GET() {
  ...
  if (!getSupabaseServiceRoleEnv()) {
    return apiError(503, "configuration_error", "Ops-API benoetigt SUPABASE_SERVICE_ROLE_KEY.")
  }
  ...
  const [uploads, reports, deletions, securityEvents] = await Promise.all([
    supabase
  ...
  if (uploads.error || reports.error || deletions.error || securityEvents.error) {
